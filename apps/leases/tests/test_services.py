...
class TestLeaseSubmission:
    def test_submit_for_review(self):
        PropertyFactory(minimum_lease_term_months=6)
        owner = UserFactory()
        lease = LeaseFactory(
            owner=owner,
    ...
    def test_submit_below_minimum_term(self):
        PropertyFactory(minimum_lease_term_months=12)
        owner = UserFactory()
        lease = LeaseFactory(
            owner=owner,
            lease_start_date=date.today(),
            lease_end_date=date.today() + timedelta(days=90),
        )
    ...
    def test_submit_non_draft_fails(self):
        PropertyFactory()
        owner = UserFactory()
        lease = LeaseFactory(owner=owner, status=Lease.Status.ACTIVE)
...
class TestLeaseReview:
    def _create_pending_lease(self):
        PropertyFactory(require_screening_for_approval=False)
        owner = UserFactory()
        lease = LeaseFactory(
            owner=owner,
            lease_start_date=date.today(),
